...
class Live : Component() {
    ...
    @Keep
    external fun stdDet(matAddr : Long ) : Double

    @Keep
    external fun initDlibLandMarks(assetManager: AssetManager?, fileName: String?) : Int

    @Keep
    external fun initDlib128Vector(assetManager: AssetManager?, fileName: String?) :Int

    @Keep
    external fun face128VectorDet(matAddr : Long) : DoubleArray
}