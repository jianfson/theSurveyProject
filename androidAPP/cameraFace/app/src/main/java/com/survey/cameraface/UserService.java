...
public class UserService {
    public static boolean signIn(String name, String password) {
        MyThread myThread = new MyThread("http://223.128.95.38:8081/BOOKCITY/loginServlet?action=login",name,password);
        try
        {
            myThread.start();
            myThread.join();
    ...
    public static boolean signUp(String name, String password) {
        MyThread myThread = new MyThread("http://223.128.95.38:8081/BOOKCITY/registServlet?action=regist",name,password);
        try
        {
            myThread.start();
            myThread.join();
...
class MyThread extends Thread
{
    ...
    public void run()
    {
        try {
            ...
            String data = "username=" + URLEncoder.encode(name, "utf-8") + "&password=" + URLEncoder.encode(password, "utf-8");//设置数据
